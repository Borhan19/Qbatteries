# -*- coding: utf-8 -*-
"""Dissipative-Time=4pi/g, Target=|1><1|, 6Cells.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1KcYjB_hLBAoZ0AGcMZQgLwKYwLOgPick
...
qutip.tensor(qutip.qeye(2),qutip.qeye(2),qutip.qeye(2),qutip.qeye(2),qutip.qeye(2),qutip.qeye(2),qutip.qeye(2))
...
def liouvillian(omega, g, gamma, beta):
    ...
    # Cooling on TLS
    L1 = np.sqrt(gamma * (N + 1)) * qutip.tensor(qutip.destroy(2),qutip.qeye(2),qutip.qeye(2),qutip.qeye(2),qutip.qeye(2),qutip.qeye(2),qutip.qeye(2))
    # Heating on TLS
...
rho_th = qutip.Qobj(qutip.tensor(qutip.basis(2,0)*qutip.basis(2,0).dag(), qutip.basis(2,0)*qutip.basis(2,0).dag(),qutip.basis(2,0)*qutip.basis(2,0).dag(),qutip.basis(2,0)*qutip.basis(2,0).dag(),qutip.basis(2,0)*qutip.basis(2,0).dag(),qutip.basis(2,0)*qutip.basis(2,0).dag(),qutip.basis(2,0)*qutip.basis(2,0).dag()))
print(rho_th)
...
rho_T_trg = qutip.tensor(qutip.Qobj(np.diag([0, 1])),qutip.Qobj(np.diag([0,1])),qutip.Qobj(np.diag([0,1])),qutip.Qobj(np.diag([0,1])),qutip.Qobj(np.diag([0,1])),qutip.Qobj(np.diag([0,1])))
...
def print_qubit_error(**args):
    ...
    for state_T in args['fw_states_T']:
        state_q_T = np.trace(np.array(state_T).reshape(2,2,2,2,2,2,2,2,2,2,2,2,2,2), axis1=0, axis2=7)
        FinalStateB=state_q_T.reshape(64,64)       
        taus.append(FinalStateB[63, 63].real)
...
for i in range(0,nt):
  FinalStateB = np.trace(np.array(optimized_dynamics.states[i]).reshape(2,2,2,2,2,2,2,2,2,2,2,2,2,2), axis1=0, axis2=7)
