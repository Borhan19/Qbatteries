# -*- coding: utf-8 -*-
"""Dissipative-NLCharger-NLBattery, Time=4pi/g, Target=|n><n|.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1rEpsjw9S5Vzso0rniLdGDEI2mxzyrrwq
...
def print_qubit_error(**args):
    ...
    for state_T in args['fw_states_T']:
        n=np.int(np.sqrt(np.shape(state_T)[0]))
        state_q_T = np.trace(np.array(state_T).reshape(n,n,n,n), axis1=0, axis2=2)        
        taus.append(state_q_T[n-1, n-1].real)
...
def chis_qubit(fw_states_T, objectives, tau_vals):
    ...
    for state_i_T in fw_states_T:
        n=np.int(np.sqrt(np.shape(rho_th)[0]))
        chi_i = qutip.Qobj(np.kron(np.identity(n),rho_T_trg))
        chis.append(chi_i)
    return chis
...
for n in range (40, 101):
  ...
  rho_trg = np.kron(rho_q_trg, rho_T_trg)
  rho_trg = qutip.Qobj(rho_trg)
  rho_th = qutip.Qobj(np.kron(np.array(qutip.basis(n,0)*qutip.basis(n,0).dag()), np.array(qutip.basis(n,0)*qutip.basis(n,0).dag())))
  ...
  time=np.zeros(nt)
  for i in range(0,nt):
    FinalRho=np.trace(np.array(optimized_dynamics.states[i]).reshape(n,n,n,n), axis1=0, axis2=2)
    Rho_f=np.zeros((n,n))
    for j in range(0,n):
      Rho_f=eigenvalues(FinalRho)[j]*(qutip.basis(n,n-(j+1))*qutip.basis(n,n-(j+1)).dag())+Rho_f
    Energy[i]=np.real(np.matrix.trace(omega*np.dot(np.array(np.dot(qutip.create(n),qutip.destroy(n))),FinalRho)))
    Ergotropy[i]=-np.real(np.matrix.trace(omega*np.dot(np.array(np.dot(qutip.create(n),qutip.destroy(n))),(Rho_f-FinalRho))))
    time[i]=(T/nt)*i
