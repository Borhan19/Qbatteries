# -*- coding: utf-8 -*-
"""Dissipative-Time=4pi/g, Target=|1><1|, 7Cells.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1YjI-K9wbuyRDZKB4Xe-ZPmIkuJJxMxHY
...
rho_T_trg = qutip.tensor(qutip.Qobj(np.diag([0, 1])),qutip.Qobj(np.diag([0,1])),qutip.Qobj(np.diag([0,1])),qutip.Qobj(np.diag([0,1])),qutip.Qobj(np.diag([0,1])),qutip.Qobj(np.diag([0,1])),qutip.Qobj(np.diag([0,1])))
...
def print_qubit_error(**args):
    ...
    for state_T in args['fw_states_T']:
        state_q_T = np.trace(np.array(state_T).reshape(2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2), axis1=0, axis2=8)
        FinalStateB=state_q_T.reshape(128,128)       
        taus.append(FinalStateB[127, 127].real)
...
def plot_iterations(opt_result):
    ...
    plt.legend()
    fig.savefig('ElectricField7cells.png')
...
for i in range(0,nt):
  FinalStateB = np.trace(np.array(optimized_dynamics.states[i]).reshape(2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2), axis1=0, axis2=8)
  FinalStateCell1=np.trace(np.array(FinalStateB).reshape(2,64,2,64), axis1=1, axis2=3)
  PreFinalStateCell2=np.trace(np.array(FinalStateB).reshape(2,2,32,2,2,32), axis1=2, axis2=5)
  FinalStateCell2=np.trace(np.array(PreFinalStateCell2).reshape(2,2,2,2), axis1=0, axis2=2)
  PreFinalStateCell3=np.trace(np.array(FinalStateB).reshape(4,2,2,2,2,2,4,2,2,2,2,2), axis1=0, axis2=6)
  FinalStateCell3=np.trace(np.array(PreFinalStateCell3).reshape(2,16,2,16), axis1=1, axis2=3)
  PreFinalStateCell4=np.trace(np.array(FinalStateB).reshape(8,2,2,2,2,8,2,2,2,2), axis1=0, axis2=5)
  FinalStateCell4=np.trace(np.array(PreFinalStateCell4).reshape(2,8,2,8), axis1=1, axis2=3)
  PreFinalStateCell5=np.trace(np.array(FinalStateB).reshape(16,2,2,2,16,2,2,2), axis1=0, axis2=4)
  FinalStateCell5=np.trace(np.array(PreFinalStateCell5).reshape(2,4,2,4), axis1=1, axis2=3)
  PreFinalStateCell6=np.trace(np.array(FinalStateB).reshape(32,2,2,32,2,2), axis1=0, axis2=3)
  FinalStateCell6=np.trace(np.array(PreFinalStateCell6).reshape(2,2,2,2), axis1=1, axis2=3)
  FinalStateCell7=np.trace(np.array(FinalStateB).reshape(64,2,64,2), axis1=0, axis2=2)
...
ax1.legend()  
fig2.savefig('EnergyvsErgotropy7cells.png')
...
file1 = open("Percentage7cells.txt","w")
